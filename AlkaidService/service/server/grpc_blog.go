...
import (
	"context"
	"fmt"
	pb "github.com/dipper/AlkaidService/protorpc/base"
	pbb "github.com/dipper/AlkaidService/protorpc/blog"
...
func init() {
	config.SetProviderService(NewRpcBlogService())
}

var myconfig = utils.Config

type RpcBlogService struct {
	*pb.BaseServiceProviderBase
}

func NewRpcBlogService() *RpcBlogService {
	return &RpcBlogService{&pb.BaseServiceProviderBase{},}
}
...
func packBlogList(data []dao.TBlog) (*pb.RpcResult, error) {
	...
	for i := 0; i < len(data); i++ {
		any, err := ptypes.MarshalAny(&pbb.BlogPro{
			Id:      data[i].Id,
...
func (rbs *RpcBlogService) Reference() string {
	return "com.skynet.dipper.commons.grpc.base.DubboBaseServiceGrpc$IBaseService"
}
